	...
	int main(int argc, char *argv[])
	{
		...
		int goal,lowestGoal,highestGoal,profiling,priority,orchPort,execTimes,selected_kernel=0;
		...
		while ( (opt = getopt_long(argc, argv, "a:g:l:h:p:n:r:o:e:k:", long_options, &long_index )) != -1 ) 
			switch (opt) 
			{
				case 'k':
					selected_kernel = atoi(optarg);
					break;
				case 'a':
